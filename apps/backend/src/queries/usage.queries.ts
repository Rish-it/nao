...
const TOTAL_LLM_INFERENCE_COST_EXPR = sql<number>`${LLM_INFERENCE_COST_EXPR.inputNoCache} + ${LLM_INFERENCE_COST_EXPR.inputCacheRead} + ${LLM_INFERENCE_COST_EXPR.inputCacheWrite} + ${LLM_INFERENCE_COST_EXPR.output}`;

const ZERO_ACTION_COSTS = {
	chatCost: 0,
	testCost: 0,
	memoryCost: 0,
	voiceCost: 0,
...
export async function createLlmInferenceCostLookup(projectId: string) {
	const table = await buildCostValuesTable(projectId);
	const joinCondition = sql`cost_lookup.provider = ${s.llmInference.llmProvider} AND cost_lookup.model_id = ${s.llmInference.llmModelId}`;
	return { table, joinCondition };
}
...
export const getMessagesUsage = async (projectId: string, filter: UsageFilter): Promise<UsageRecord[]> => {
	...
	const records = [...allDates].map((date) => {
		...
		const actionCosts = actionCostsByDate.get(date) ?? ZERO_ACTION_COSTS;
		return {
			date,
			...actionCosts,
			...
			whatsappMessageCount: row?.whatsappMessageCount ?? 0,
	...
	return fillMissingDates(records, granularity);
};
...
export const getUsedProviders = async (projectId: string): Promise<LlmProvider[]> => {
	...
	const providers = new Set([...messageRows.map((row) => row.provider), ...inferenceRows.map((row) => row.provider)]);
...
async function getActionCostsByDate(
	...
	const costLookup = await createLlmInferenceCostLookup(projectId);

	const rows = await db
		.select({
			date: dateExpr,
			chatCost: sql<number>`sum(case when ${s.llmInference.type} in ('chat', 'compaction', 'title_generation') then ${TOTAL_LLM_INFERENCE_COST_EXPR} else 0 end)`,
			testCost: sql<number>`sum(case when ${s.llmInference.type} = 'test' then ${TOTAL_LLM_INFERENCE_COST_EXPR} else 0 end)`,
			memoryCost: sql<number>`sum(case when ${s.llmInference.type} = 'memory_extraction' then ${TOTAL_LLM_INFERENCE_COST_EXPR} else 0 end)`,
